...
async function generateHmacToken(secret: string): Promise<string> {
...
export async function GET(req: Request): Promise<Response> {
  ...
  if (p) {
    ...
    try {
      const upstream = await fetch(
        ...
        { headers: { "User-Agent": "NextJS-SegmentProxy/1.0" } }
      );
  ...
  try {
    ...
    // Local dev falls back to /api/stream?p= (no base64 decode needed there).
      ...
      text = text.replace(
        ...
        (_match, enc: string) => {
          try {
            ...
            return decoded.includes(".ipcamlive.com") ? decoded : _match;
          } catch { return _match; }
        }
      );
    } else {
      text = text.replace(
        /https?:\/\/[^/\s"']+\/(?:api\/stream|stream\/ts)\?p=/g,
        "/api/stream?p="
      );
    }
